...
4. **Medium — public quote actor substitution.** `api.quote_order` accepted a caller-selected account UUID for coupon-use and first-order eligibility. The migration binds public calls to `auth.uid()` while preserving the service-role path used by order placement. **Applied to production on 12 September 2026 as `20260912152133_restrict_quote_actor_identity`.** Tests against the deployed function confirmed anonymous actor spoofing is rejected, authenticated identity is used, and the service-role actor path is preserved. The temporary coupon fixture was rolled back and verified absent; existing function ownership, grants, stability and search path were preserved. The repository filename matches the version assigned by Supabase. This finding concerns quote eligibility/privacy; no successful order-price bypass was demonstrated.
...
- Local logs: `docs/.visual-audit-assets/final-feature.log`, `quick-view-final.log`, `checkout-journeys-final.log`, and `coverage-release.log`. Journey screenshots wait for the checkout form and verified prices before capture.
- [GitHub CI for the application changes](https://github.com/miskatulanika21/nongor-brand/actions/runs/34687662658) passed quality checks, fresh migration-chain application, database integration tests and concurrency tests. CodeQL also passed. The isolated quote-identity SQL guard test separately passed and rolled back its fixture. The local Docker daemon is unavailable, so fresh database validation was performed by CI.
- Vercel preview deployment succeeded. Its post-deploy workflow skipped the actual smoke/axe steps; that green wrapper is not counted as browser coverage. Browser results above come from the locally executed production-mode build.
- After applying the quote-identity migration, six read-only Chromium checks passed against `https://nongorr.com`: verified cart arithmetic, home/CSP, database health, catalog, cart pricing and login entry. All 95 local migration versions exactly match production history. Evidence: `docs/.visual-audit-assets/migration-production-smoke.log`.
