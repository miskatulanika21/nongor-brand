...
/**
 * Canonical audit action names live in the isomorphic `audit-shared` module
 * (`AUDIT_ACTIONS`), which is also the display catalog read by the admin Audit
 * Logs page. Importing the type from there keeps the writer's contract and the
 * viewer's label map from ever drifting. Extend the list in `audit-shared.ts`.
 */
