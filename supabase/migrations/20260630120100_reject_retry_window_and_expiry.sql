-- Stage 3 fix pass — payment-rejected reservation lifecycle (BUG-03, corrected).
--
-- The deep review proposed releasing the reservation when a payment is rejected.
-- That is UNSAFE: payment_rejected is retryable (payment_rejected → payment_submitted
-- → confirmed), and private.consume_reservations needs the still-active hold to
-- decrement stock at confirm. Releasing on reject would silently skip the stock
-- decrement on a retried+confirmed order (oversell).
--
-- Correct behaviour:
--   * On transition to payment_rejected, REFRESH the hold window (orders +
--     active reservation rows) so the customer keeps a fair retry window and the
--     held stock isn't lazily released out from under them mid-retry.
--   * Add payment_rejected to api.expire_reservations so an ABANDONED rejected
--     order is reclaimed (status→expired, reservations released) after its window,
--     instead of holding ready stock until the original TTL with a stale 'active'
--     row that the sweep never touched. payment_submitted is still NOT swept (it
...
-- Sweep abandoned rejected orders too (not payment_submitted — that awaits admin).
...
BEGIN
  FOR rec IN
    SELECT id, status
    FROM public.orders
    WHERE status IN ('pending_payment','pending_confirmation','payment_rejected')
