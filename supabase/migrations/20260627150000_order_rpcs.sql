-- Stage 3 Pass 3a — server-authoritative pricing & order creation RPCs.
--
-- api.quote_order  : public read. Prices a cart from the DB, returns per-line
--                    availability + a drift token. The honest pre-submit total.
-- api.place_order  : service-role only (the app server fn adds CSRF/rate-limit/
--                    auth). One transaction: race-safe idempotency (ON CONFLICT),
--                    deterministic product locking (deadlock-free), server-side
--                    pricing (client totals ignored), oversell guard via
--                    available_qty, price-drift detection, soft reservation,
--                    order+items+payment+status-history, guest token.
--
-- Coupons are NOT handled yet (discount is always 0); they land in P5. Pricing
-- shared by both RPCs via private.price_lines so quote and place never diverge.
-- Design: docs/stage-3-design.md (v2) §4.

-- ── Shipping (free-delivery keyed on PRE-discount subtotal — pinned rule) ─────
CREATE OR REPLACE FUNCTION private.compute_shipping(p_zone text, p_subtotal integer)
RETURNS integer
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
SET search_path = ''
AS $$
DECLARE fee integer; thresh integer;
BEGIN
  SELECT
    CASE p_zone WHEN 'dhaka' THEN delivery_fee_dhaka
                WHEN 'major' THEN delivery_fee_major
                ELSE delivery_fee_outside END,
    free_delivery_threshold
  INTO fee, thresh
  FROM public.site_settings WHERE id = 1;

  IF thresh IS NOT NULL AND p_subtotal >= thresh THEN RETURN 0; END IF;
  RETURN COALESCE(fee, 0);
END;
$$;

-- ── Price a cart from the DB (single source of truth for quote + place) ──────
-- Input lines: [{ code, size?, qty }]. Emits one priced object per input line,
-- order preserved, with current price, availability and visibility.
  ...
  FOR el IN SELECT value FROM jsonb_array_elements(p_lines) AS value LOOP
    ...
    IF prod.id IS NULL THEN
      out := out || jsonb_build_object('code', el->>'code', 'size', v_size,
        'qty', v_qty, 'found', false, 'visible', false);
      CONTINUE;
    END IF;
    ...
    out := out || jsonb_build_object(
      ...
      'size', v_size, 'qty', v_qty, 'unit_price', prod.unit_price,
      'line_total', prod.unit_price * v_qty, 'available', v_avail,
      'visible', prod.visible, 'found', true);
  END LOOP;
  RETURN out;
END;
$$;
...
-- ── Public quote ─────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION api.quote_order(p_lines jsonb, p_zone text)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
SET search_path = ''
AS $$
DECLARE priced jsonb; subtotal integer; shipping integer; canon text;
BEGIN
  ...
  RETURN jsonb_build_object(
    'lines', priced, 'subtotal', subtotal, 'discount', 0,
    'shipping_fee', shipping, 'total', subtotal + shipping,
    'quote_token', md5(COALESCE(canon,'') || '#' || subtotal), 'zone', p_zone);
END;
$$;

REVOKE ALL ON FUNCTION api.quote_order(jsonb, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION api.quote_order(jsonb, text) TO anon, authenticated, service_role;

-- ── Place order (service-role only) ──────────────────────────────────────────
...
DECLARE
  ...
  v_guest_token text; v_guest_hash text; pidlist uuid[];
BEGIN
  ...
  FOR e IN SELECT value FROM jsonb_array_elements(priced) AS value LOOP
    ...
    IF (e->>'available')::int < (e->>'qty')::int THEN
