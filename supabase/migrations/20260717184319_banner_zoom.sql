-- ══════════════════════════════════════════════════════════════════════════════
-- Banner zoom — Focal Studio Phase 2
--
-- Zoom magnifies the image around its focal point (scale 1..3), letting an admin
-- crop tighter without a re-cropped file — the hero applies it as a CSS
-- transform: scale() with transform-origin at the focal point. Existing rows
-- backfill to 1.0 (no zoom), so nothing changes for banners already framed.
-- ══════════════════════════════════════════════════════════════════════════════
...
-- get_active_banners: add zoom to the public payload (explicit jsonb_build_object).
...
AS $$
  ...
  FROM (
    SELECT b.sort_order, b.created_at, jsonb_build_object(
      ...
      'focal_x', b.focal_x, 'focal_y', b.focal_y, 'zoom', b.zoom
    ) AS r
...
-- upsert_banner: accept + clamp zoom (1..3) on create and edit.
...
DECLARE
  ...
  v_zoom          numeric := LEAST(3, GREATEST(1, COALESCE((p_banner->>'zoom')::numeric, 1)));
BEGIN
  ...
  IF v_id IS NULL THEN
    INSERT INTO public.banners (
      ...
      focal_x, focal_y, zoom, updated_by
    ) VALUES (
      ...
      v_focal_x, v_focal_y, v_zoom, p_actor
    )
